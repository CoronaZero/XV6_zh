...
函数 bmap (4810) 首先处理简单的情况：前 NDIRECT 个块列在 inode 自身 (4815-4819) 中。接下来的 NINDIRECT 个块列在 ip->addrs[NDIRECT] 处的间接块中。bmap 读取间接块 (4826)，然后从块中正确位置读取块号 (4827)。如果块号超过 NDIRECT+NINDIRECT，bmap 会 panic：调用者有责任不请求超出范围的块号。
...
readi 和 writei 都以检查 ip->type == T_DEV 开始。这种情况处理其数据不驻留在文件系统中的特殊设备；我们将在文件描述符层回到这种情况。
